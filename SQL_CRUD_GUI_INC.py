...
class LookGUI:
    def __init__(self, master):
        ...
        # TODO      a button titled "Search First Name" that calls self.search_first *DONE
        # TODO      a button titled "Search Last Name" that calls self.search_last  DONE********
        # TODO      a button titled "Main Menu" that closes the window and returns to the main menu *DONE
        self.search_button1 = tkinter.Button(self.bottom_frame, text='Search First Name', command=self.search_first)
        self.search_button2 = tkinter.Button(self.bottom_frame, text='Search Last Name', command=self.search_last)
        self.menu_button = tkinter.Button(self.bottom_frame, text='Main Menu', command=self.look.destroy)
    ...
    def search_first(self):
        ...
        temp = find_customer_by_name(name, False)
        return_data_as_string(temp)
        ...
        # TODO      set the Boolean parameter for find_customer_by_name() as indicated in the function description  *DONE*
...
class AddGUI:
    def __init__(self, master):
        ...
        # TODO place each label and entry box into the corresponding frame (defined above)  *DONE*
        # TODO set appropriate widths for the entry boxes
        self.first_label = tkinter.Label(self.first_name_frame, text='Enter customers first name: ')
        self.first_entry = tkinter.Entry(self.first_name_frame, width=20, font="TkDefaultFont")

        self.last_label = tkinter.Label(self.last_name_frame, text='Enter customers last name: ')
        self.last_entry = tkinter.Entry(self.last_name_frame, width=20, font="TkDefaultFont")

        self.phone_label = tkinter.Label(self.last_name_frame, text='Enter customers phone number: ')
        self.phone_entry = tkinter.Entry(self.last_name_frame, width=20, font="TkDefaultFont")

        self.email_label = tkinter.Label(self.last_name_frame, text='Enter customers email address: ')
        self.email_entry = tkinter.Entry(self.last_name_frame, width=20, font="TkDefaultFont")
        ...
        # TODO create a Label in self.result_frame with width of 45 that will display the contents of self.message  *DONE*
        self.message = tkinter.StringVar()
        self.result = tkinter.Label(self.result_frame, textvariable=self.message, width=45, font="TkDefaultFont")
        ...
        # TODO     the Add button should call self.add_customer  *DONE*
        # TODO     the Main Menu button should close the window and return to the master window  *DONE*
        self.add_button = tkinter.Button(self.button_frame, text='Add', command=self.add_customer)
        self.menu = tkinter.Button(self.button_frame, text='Main Menu', command=self.add.destroy)
    ...
    def add_customer(self):
        ...
        if len(first_name and last_name and phone and email) >= 1:
            customer_db = None
            try:
                ...
                # TODO remember to commit your changes  *DONE*
            except sqlite3.Error as err:
                tkinter.messagebox.showinfo('Database Error', f'{err}')
                ...
                if customer_db is not None:
                    ...
                    # TODO 3. close the database connection *DONE
        else:
            self.message.set('Data was not added due to incomplete data')
...
class ChangeGUI:
    def __init__(self, master):
        ...
        # TODO     Update should call self.change_customer      *DONE
        # TODO     Cancel should call self.cancel_update        *DONE
        # TODO     Main Menu should close the window and return to the master window        *DONE
        ...
        self.update = tkinter.Button(self.button_frame, text='Update', command=self.change_customer)
        self.cancel = tkinter.Button(self.button_frame, text='Cancel', command=self.cancel_update)
        self.menu = tkinter.Button(self.button_frame, text='Main Menu', command=self.change.destroy)
...
class DeleteGUI:
    ...
    def remove(self):
        ...
        try:
            conn = sqlite3.connect('customers.db')
            cur = conn.cursor()
            cur.execute('SELECT * FROM Customers WHERE CustomerID == ?', (customer_db,))
...
def find_customer_by_name(name, first_name):
    ...
    try:
        conn = sqlite3.connect('customers.db')
        cur = conn.cursor()
        cur.execute(f'SELECT * Customers WHERE FirstName LIKE {pattern_to_match}')
        results.append(cur.fetchall())

        # TODO:
        #  1. establish a connection to the database *DONE
        #  2. SELECT all fields from Customers that match the name to FirstName or LastName depending on first_name *DONE
        #  3. fetch the resulting data into the list named results *DONE
...
def find_customer_by_id(customer_id):
    ...
    try:
        conn = sqlite3.connect('customers.db')
        cur = conn.cursor()
        cur.execute(F'SELECT * Customers WHERE CustomerID == {customer_id}')
        results.append(cur.fetchall())
        return customer_id
        # TODO:
        #  1. establish a connection to the database *DONE
        #  2. SELECT all fields from the Customers database that match the given customer_id *DONE
        #  3. fetch all resulting data into the list named results *DONE
