...
count = 0  # Lets I pass the first if statement when there are no digits found
for i in name:
    if i.isdigit():
        count = 1  # when the code gets to this part, no matter how many digits there are this counter, it will always be one showing that there is a digit in the string
        print("Name contains a number")
        break
if count == 0:
    print("Name does not contain number")
