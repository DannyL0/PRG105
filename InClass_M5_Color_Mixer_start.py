...
def get_color():
    color = input("Enter red/yellow/blue: ")
    color = color.upper()

    # validate the input: test for BAD data -- if its BAD, make them re-enter the data
    # if the data is good, there is nothing more to do
    while color != "RED" and color != "YELLOW" and color != "BLUE":
        color = input("Bad data. Please enter red/yellow/blue: ")
        color = color.upper()
...
def check_colors(one, two):
    ...
    if one == "BLUE":
        ...
        if two == "BLUE":
            ...
            result = "ORANGE"
    else:  # the only other choice for the first color is yellow
        if two == "BLUE":
            result = "GREEN"
        else:  # the only other choice is red
            result = "ORANGE"
    return result
